...
```
- `my-mysql`：替换为你的容器名或 ID
- `-u root`：以 root 用户登录（可换成你的用户名）
- `-p`：会提示你输入密码
...
#### 数据备份
- 打包data: `mysqldump -h 127.0.0.1 -u root -p database_A > database_A.sql`
- (Docker版)打包data: 在非docker版基础上，推出容器，加上`docker cp container_name:/database_A.sql ./database_A.sql`
- (Docker版)load data: `sudo docker exec -i container_name mysql -u root -pYourPassword database_B < database_A.sql` (注意`-pYourPassword`不要加空格)
- 复制数据从A表到B表：`INSERT INTO sheet_B SELECT * FROM sheet_A;`

#### emoji支持
`utf8mb4_0900_ai_ci` 是 **MySQL 8.0+ 新增的一种排序规则（collation）**，属于 `utf8mb4` 字符集下的一个选项。它的名字可以拆解为：

| 部分 | 含义 |
|------|------|
| `utf8mb4` | 字符集：支持 4 字节 UTF-8（包括 emoji） |
| `0900` | 基于 **Unicode 9.0.0** 的排序规则版本 |
| `ai` | **Accent Insensitive**（不区分重音符号）<br>例如：`é` = `e`，`ñ` = `n` |
| `ci` | **Case Insensitive**（不区分大小写）<br>例如：`A` = `a` |

---

和常用 `utf8mb4_unicode_ci` 有什么区别？

| 对比项 | `utf8mb4_unicode_ci` | `utf8mb4_0900_ai_ci` |
|--------|------------------------|------------------------|
| Unicode 版本 | 较旧（MySQL 5.x 时代） | **Unicode 9.0.0**（更新、更准） |
| 重音处理 | 区分重音（`é ≠ e`） | **不区分重音**（`é = e`）✅ |
| 大小写处理 | 不区分（`A = a`） | 不区分（`A = a`） |
| 性能 | 稍慢（旧算法） | 稍快（新算法优化） |
| MySQL 版本 | 5.5+ | **8.0+**（仅支持 8.0 及以上） |
...
CREATE TABLE messages (
    ...
    content VARCHAR(255) 
        ...
        COLLATE utf8mb4_unicode_ci  -- 或 utf8mb4_0900_ai_ci（MySQL 8.0+）
);
```
