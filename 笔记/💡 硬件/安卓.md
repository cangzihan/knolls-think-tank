...
已知应用场景：
- 部分App会检测机型
- 软银Softbank的基站会根据机型限制决定是否给手机信号
    - 特征：SIM卡只在插入后会有一个短暂时间有信号，随后无服务

首先安装 Magisk 模块 「MagiskHide Props Config」。

以下是具体操作步骤：

### 1. 找个“窗口”：安装终端 App
如果你的手机里还没有终端软件，请去下载一个：
* **推荐：Termux** (最强大的安卓终端)。
* **备选：Terminal Emulator** (比较老旧，但也够用)。
* 或者在 Google Play 搜索“Terminal”随手下个分高点的也行。

### 2. 召唤权限：进入“上帝模式”
打开 Termux（或其他终端 App），你会看到一个黑框。依次输入以下指令：

1.  输入 **`su`** 并回车。
    * **关键：** 这时 Magisk 会弹出权限申请窗口，请点击 **「允许 / Grant」**。你的命令提示符通常会从 `$` 变成 `#`，这意味着你现在拥有了最高权限。
2.  输入 **`props`** 并回车。
    * 稍微等几秒，就会进入 MagiskHide Props Config 的彩色文字菜单。

### 3. 开始伪装：修改机型指纹
在菜单里，按照数字提示进行选择（通常是以下流程）：

1.  选择 **`1`** (Edit device fingerprint)。
2.  选择 **`f`** (Pick a certified fingerprint)。
3.  找到 **`Google`**（如果是数字列表，就选对应的数字）。
4.  找到 **`Pixel 6 Pro (12)`**。
    * **为什么选它：** 因为软银官方完美支持这两款机型，它们的“指纹”在软银基站那里是 VIP 级别的。
5.  确认选择，然后输入 **`y`**。
6.  **最后一步：** 程序会问你是否重启，输入 **`y`** 让手机重启。

---

### 重启后要做的事

重启后，手机会以“Pixel”的身份去尝试连接软银基站。

1.  **观察信号：** 看看那个“闪断”的现象还会不会发生。
2.  如果一开始就没有信号，那么就把卡拔了换个卡槽

::: warning ⚠️ 温馨提示
* **如果后悔了：** 随时再次进入 `props` 菜单，选择 **`r`** (Reset) 就可以恢复原样。
:::
